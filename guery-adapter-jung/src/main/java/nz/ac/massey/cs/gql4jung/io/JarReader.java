...
public class JarReader {
    ...
    public synchronized DirectedGraph<TypeNode, TypeRef> readGraph() throws GraphIOException {
        ...
        if (jars.size() == 0) {
            ...
            return new DirectedSparseGraph<TypeNode, TypeRef>();
        }
        ...
        final DirectedGraph<TypeNode, TypeRef> graph = new DirectedSparseGraph<TypeNode, TypeRef>();
        final Map<String, TypeNode> vertices = new HashMap<String, TypeNode>();
    ...
    private void addVertex(DirectedGraph<TypeNode, TypeRef> graph, Classfile classfile, int id, Map<String, TypeNode> vertices, Map<Classfile, String> containerMapping) {
        TypeNode v = new TypeNode();
    ...
    private void addEdge(DirectedGraph<TypeNode, TypeRef> graph, Node start, Node end, boolean isUses, int counter, Map<String, TypeNode> vertices) {
        TypeNode source = findVertex(start, vertices);
        TypeNode target = findVertex(end, vertices);
        if (target != null) { // this is possible - reference to external class
            ...
            if (addEdge && this.removeDuplicateEdges) {
                for (TypeRef e : graph.getOutEdges(source)) {
                    // TODO, FIXME
    ...
    private TypeNode findVertex(Node n, Map<String, TypeNode> vertices) {
