...
public abstract class AbstractChainCompressionCacheTest {
    ...
    protected static void save(DirectedGraph<TypeNode, TypeRef> g, String name) throws Exception {
        if (GENERATE_DOT_FILES) {
            ...
            Function<TypeNode, String> vertexLabels = new Function<TypeNode, String>() {
                @Override
                public String apply(TypeNode s) {
                    return s.getFullname().replace('.', '_').replace('$', '_');
                }
            };
            Function<TypeRef, String> edgeLabels = new Function<TypeRef, String>() {
                @Override
                public String apply(TypeRef s) {
                    return s.getType();
                }
            };
