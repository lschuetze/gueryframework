 ...
 */
final class GroupByAggregation<V extends Vertex<E>, E extends Edge<V>> implements MotifInstanceAggregation<V,E> {

	@Override
	public Object getGroupIdentifier (MotifInstance<V,E> instance) {
		final Motif<V,E> motif = instance.getMotif();
		List<GroupByClause<V>> clauses = motif.getGroupByClauses();
		
		if (clauses.size()==1) {
			return getValue(clauses.get(0),instance);
		}
		else {
