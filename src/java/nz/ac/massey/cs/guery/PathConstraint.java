 ...
 */
public class PathConstraint<V extends Vertex<E>,E extends Edge<V>> implements Constraint {
	...
	public Iterator<? extends Path<V,E>> getPossibleSources(final DirectedGraph<V,E> g,final V target,PathFinder<V, E> finder) {
	...
	public Iterator<? extends Path<V,E>>  getPossibleTargets(final DirectedGraph<V,E> g, final V source,PathFinder<V, E> finder){
		if (this.isNegated()) throw new IllegalStateException("negated path constraints can only be checked if source and target are known");
		return finder.findLinks(g,source,this.minLength,this.maxLength, true, filter,computeAll);
	}

	public Iterator<? extends Path<V,E>> check(final DirectedGraph<V,E> g, final V source, final V target,PathFinder<V, E> finder){
		if (this.negated) {
